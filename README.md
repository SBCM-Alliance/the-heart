...
graph TD
    ...
    Valve -- "歪み &gt; 10.0<br>(遮断・徴収)" --> Heart(("<b>The Heart</b><br>Liquidity Pool"))
    Valve -- "正常<br>(承認)" --> Tokyo["<b>Tokyo</b><br>(Predator Block)"]
    
    Heart -->|"Systole (収縮)"| Target{"<b>Triage Logic</b><br>(生存判定)"}
    
    Target -- "瀕死 (HP &lt; 30%)<br>輸血 (Special Quest)" --> Yubari["<b>Yubari</b><br>(Dying Block)"]
    Target -- "瀕死<br>輸血" --> Akita["<b>Akita</b><br>(Dying Block)"]
